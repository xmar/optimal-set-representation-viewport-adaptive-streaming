...
class SolverConfiguration:
    ...
    def GetConfigurationStr(self):
        ...
        s += 'minSurfaceBitrate={}\n'.format(float(self.minSurfaceBitrate)/float(self.Bratio))
        s += 'maxSurfaceBitrate={}\n'.format(float(self.maxSurfaceBitrate)/float(self.Bratio))
    ...
    def SolutionId(self, outputId=False):
        return '{}_{:.6f}_{:.6f}_{:.6f}_{:.6f}_{:.6f}{}'.format(self.nbQer, float(self.segmentDuration), float(self.minSegQuality), float(self.minSurfaceBitrate)/float(self.Bratio) if not outputId else float(self.minSurfaceBitrate), float(self.maxSurfaceBitrate)/float(self.Bratio) if not outputId else float(self.maxSurfaceBitrate), float(self.bitrateRatio),
          '_{:.2f}'.format(float(Bratio)) if outputId else '')
    ...
    def LatexPlotName(self, fullId=False):
        return 'plot_heatmap_'+(self.OutputDirId() + '_{:.6f}_'.format(float(self.segmentDuration)) if fullId else '')+self.SolutionId(fullId)+'.tex'
    def PostprocessingName(self, fullId=False):
        return 'postprocessing_'+(self.OutputDirId() + '_{:.6f}_'.format(float(self.segmentDuration)) if fullId else '')+self.SolutionId(fullId)+'.txt'
    def PercentileName(self, fullId=False):
        return 'percentile_'+(self.OutputDirId() + '_{:.6f}_'.format(float(self.segmentDuration)) if fullId else '')+self.SolutionId(fullId)+'.txt'
    def RawQualityName(self, fullId=False):
        return 'rawQuality_'+(self.OutputDirId() + '_{:.6f}_'.format(float(self.segmentDuration)) if fullId else '')+self.SolutionId(fullId)+'.txt'
    def AverageQualityName(self):
        return 'averageOptimal_'+ (self.OutputDirId() + '_{:.6f}_'.format(float(self.segmentDuration)))+self.SolutionId(True)+'.txt'
...
def GetRawQualities(pathToRawQualities, Bratio):
    opti = []
    heuri = []
    random = []
    with open(pathToRawQualities, 'r') as i:
        ...
        for line in i:
            if first:
                ...
                values = line.split(' ')
                opti.append(float(values[0])*Bratio)
                heuri.append(float(values[1])*Bratio)
                random.append(float(values[2])*Bratio)
        averageOpti = sum(opti)/len(opti)
    return (opti, heuri, random, averageOpti)
...
if __name__ == '__main__':
    ...
    with open(outputDir+'/mergedPostprocessing.txt', 'w') as postprocessingO:
        ...
        for Bratio in config['ManagerConfig']['Bratio'].split(','):
                            ...
                            for minSurfaceBitrate in config['ManagerConfig']['minSurfaceBitrate'].split(','):
                                for maxSurfaceBitrate in config['ManagerConfig']['maxSurfaceBitrate'].split(','):
                                    for minSegQuality in config['ManagerConfig']['minSegQuality'].split(','):
                                        ...
                                        solverConfig = SolverConfiguration(Bratio, selectionPerSegment, nbQer, segmentDuration, minSurfaceBitrate, maxSurfaceBitrate, minSegQuality, bitrateRatio, nbTheta, nbPhi, nbHDim, nbVDim, dimMin, dimMax, nbHPixels, nbVPixels, viewportHAngle, viewportVAngle, pathToTraces, pathToTracesHash, pathToPrecomputedIntersection, pathToPrecomputedSegments)
                                        ...
                                        with open(solverConfig.PathToPercentileName(), 'r') as i:
                                            with open('{}/{}'.format(outputDir, solverConfig.PercentileName(True)), 'w') as o:
                                                firstLine = True
                                                for line in i:
                                                    if firstLine:
                                                        o.write(line)
                                                        ...
                                                        for value in line[:-1].split(' '):
                                                            if not firstValue:
                                                                o.write(' ')
                                                                o.write('{}'.format(float(value)*float(Bratio)))
                                                            else:
                                                                firstValue = False
                                                                o.write(value)
                                                        o.write('\n')
                                        shutil.copyfile(solverConfig.PathToRawQuality(), '{}/{}'.format(outputDir, solverConfig.RawQualityName(True)))
                                        rawQualities = GetRawQualities(solverConfig.PathToRawQuality(), float(Bratio))
                                        optiRawQualities += rawQualities[0]
                                        heuriRawQualities += rawQualities[1]
                                        randomRawQualities += rawQualities[2]
                                        with open('{}/{}'.format(outputDir, solverConfig.AverageQualityName()), 'w') as o:
                                            o.write('{}\n'.format(rawQualities[3]))
